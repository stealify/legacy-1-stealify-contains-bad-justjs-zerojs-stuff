const { epoll } = stealify.library('epoll')
const { http } = stealify.library('http')
const { sys } = stealify.library('sys')
const { net } = stealify.library('net')
...
const { F_GETFL, F_SETFL } = stealify.sys
const { IPPROTO_TCP, O_NONBLOCK, TCP_NODELAY, SO_KEEPALIVE, SOMAXCONN, AF_INET, SOCK_STREAM, SOL_SOCKET, SO_REUSEADDR, SO_REUSEPORT, SOCK_NONBLOCK } = stealify.net
const { setInterval } = stealify
...
class Socket {
  ...
  onEvent (fd, event) {
    ...
    if (count < 0) {
      stealify.error(`parse failed ${count}`)
      this.close()
      return true
    }
    ...
    if (count === 1) {
      ...
      if (remaining > 0) {
        if (remaining === bytes) {
          ...
          if (from > 0) {
            stealify.print(`copyFrom ${remaining} bytes from ${from} to 0`)
...
class Server {
  ...
  close () {
    loop.remove(this.fd)
    net.close(this.fd)
    stealify.clearInterval(this.timer)
  }
