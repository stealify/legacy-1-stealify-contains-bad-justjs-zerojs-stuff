...
const { html } = stealify.library('html')
...
function setupSocket (sock, config) {
  function compile (query, onComplete) {
    ...
    let readString = stealify.sys.readString
    if (query.htmlEscape) {
  ...
  function onMessage () {
    ...
    if (type === ErrorResponse) {
      ...
      //stealify.print(JSON.stringify(parser.errors, null, '  '))
      return
    }
  ...
  sock.onClose = () => {
    stealify.error('pg socket closed')
  }
