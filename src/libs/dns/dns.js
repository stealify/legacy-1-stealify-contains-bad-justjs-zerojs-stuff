...
const dnsServer = stealify.env().DNS_SERVER || '127.0.0.11'
...
function readHosts () {
  ...
  if (!isFile(fileName)) {
    stealify.error(`${fileName} not found`)
    return { ipv4, ipv6 }
  }
...
function lookup (query = 'www.google.com', onRecord = () => {}, address = dnsServer, port = 53, buf = new ArrayBuffer(65536)) {
  ...
  if (rc === -1) {
    const errno = stealify.sys.errno()
    onRecord(new Error(`Error sending ${query} to ${address}: ${stealify.sys.strerror(errno)} (${errno})`))
    loop.remove(fd)
    net.close(fd)
    return
  }
  const timer = stealify.setTimeout(() => {
