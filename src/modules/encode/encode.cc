...
size_t stealify::encode::hex_decode(char* buf,
                         size_t len,
                         const char* src,
                         const size_t srcLen) {
  size_t i;
...
size_t stealify::encode::base64_decoded_size(const char* src, size_t size) {
  if (size == 0)
    return 0;
...
bool stealify::encode::base64_decode_group_slow(char* dst, const size_t dstlen,
...
size_t stealify::encode::base64_decode_fast(char* dst, const size_t dstlen,
                          const char* src, const size_t srclen,
...
size_t stealify::encode::base64_decode(char* dst, const size_t dstlen,
                     const char* src, const size_t srclen) {
  const size_t decoded_size = base64_decoded_size(src, srclen);
  return base64_decode_fast(dst, dstlen, src, srclen, decoded_size);
}

size_t stealify::encode::base64_encode(const char* src,
                            size_t slen,
                            char* dst,
                            size_t dlen) {
  // We know how much we'll write, stealify make sure that there's space.
...
void stealify::encode::HexEncode(const FunctionCallbackInfo<Value> &args) {
...
void stealify::encode::Base64Encode(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<ArrayBuffer> absource = args[0].As<ArrayBuffer>();
...
void stealify::encode::Init(Isolate* isolate, Local<ObjectTemplate> target) {
