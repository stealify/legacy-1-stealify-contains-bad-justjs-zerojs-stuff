...
void stealify::crypto::Create(const FunctionCallbackInfo<Value> &args) {
  ...
  stealify::crypto::hasher* hasher = new stealify::crypto::hasher();
  ...
  hasher->ctx = ctx;
  stealify::crypto::hashers[stealify::crypto::hcount] = hasher;
  args.GetReturnValue().Set(Integer::New(isolate, stealify::crypto::hcount++));
}

void stealify::crypto::Update(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  int argc = args.Length();
  if (argc < 1) {
    args.GetReturnValue().Set(Integer::New(isolate, SHORT_ARGS));
  ...
  stealify::crypto::hasher* hasher = stealify::crypto::hashers[Local<Integer>::Cast(args[0])->Value()];
...
void stealify::crypto::Digest(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  int argc = args.Length();
  if (argc < 1) {
    args.GetReturnValue().Set(Integer::New(isolate, SHORT_ARGS));
...
void stealify::crypto::Reset(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  int argc = args.Length();
  if (argc < 1) {
    args.GetReturnValue().Set(Integer::New(isolate, SHORT_ARGS));
...
void stealify::crypto::Free(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  int argc = args.Length();
  if (argc < 1) {
    args.GetReturnValue().Set(Integer::New(isolate, SHORT_ARGS));
...
void stealify::crypto::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  Local<ObjectTemplate> module = ObjectTemplate::New(isolate);

  if (!stealify::crypto::initialized) {
    ...
    stealify::crypto::initialized = true;
  }
