...
void stealify::fs::Unlink(const FunctionCallbackInfo<Value> &args) {
...
void stealify::fs::Realpath(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value fname(isolate, args[0]);
...
void stealify::fs::Utime(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  String::Utf8Value fname(isolate, args[0]);
...
void stealify::fs::Symlink(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::fs::Mknod(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  String::Utf8Value target(isolate, args[0]);
...
void stealify::fs::Mkfifo(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  String::Utf8Value target(isolate, args[0]);
...
void stealify::fs::Mount(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::fs::Umount(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  String::Utf8Value target(isolate, args[0]);
  int flags = 0;
...
void stealify::fs::Open(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::fs::Chmod(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
...
void stealify::fs::Chown(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  int fd = Local<Integer>::Cast(args[0])->Value();
...
void stealify::fs::Ioctl(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::fs::Ftruncate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  int fd = args[0]->Int32Value(context).ToChecked();
...
void stealify::fs::Fstat(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  int fd = args[0]->Int32Value(context).ToChecked();
...
void stealify::fs::Chdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::fs::Rmdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  String::Utf8Value path(isolate, args[0]);
  int rc = rmdir(*path);
...
void stealify::fs::Rename(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  String::Utf8Value source(isolate, args[0]);
...
void stealify::fs::Mkdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  String::Utf8Value path(isolate, args[0]);
...
void stealify::fs::Lseek(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  String::Utf8Value path(isolate, args[0]);
...
void stealify::fs::Readdir(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  String::Utf8Value path(isolate, args[0]);
...
void stealify::fs::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  Local<ObjectTemplate> fs = ObjectTemplate::New(isolate);
  SET_METHOD(isolate, fs, "open", stealify::fs::Open);
  SET_METHOD(isolate, fs, "unlink", stealify::fs::Unlink);
  SET_METHOD(isolate, fs, "symlink", stealify::fs::Symlink);
  SET_METHOD(isolate, fs, "ioctl", stealify::fs::Ioctl);
  SET_METHOD(isolate, fs, "rmdir", stealify::fs::Rmdir);
  SET_METHOD(isolate, fs, "ftruncate", stealify::fs::Ftruncate);
  SET_METHOD(isolate, fs, "rename", stealify::fs::Rename);
  SET_METHOD(isolate, fs, "mkdir", stealify::fs::Mkdir);
  SET_METHOD(isolate, fs, "fstat", stealify::fs::Fstat);
  SET_METHOD(isolate, fs, "lseek", stealify::fs::Lseek);
  SET_METHOD(isolate, fs, "readdir", stealify::fs::Readdir);
  SET_METHOD(isolate, fs, "chdir", stealify::fs::Chdir);
  SET_METHOD(isolate, fs, "mount", stealify::fs::Mount);
  SET_METHOD(isolate, fs, "umount", stealify::fs::Umount);
  SET_METHOD(isolate, fs, "mknod", stealify::fs::Mknod);
  SET_METHOD(isolate, fs, "mkfifo", stealify::fs::Mkfifo);
  SET_METHOD(isolate, fs, "realpath", stealify::fs::Realpath);
  SET_METHOD(isolate, fs, "utime", stealify::fs::Utime);

  SET_METHOD(isolate, fs, "chmod", stealify::fs::Chmod);
  SET_METHOD(isolate, fs, "chown", stealify::fs::Chown);
