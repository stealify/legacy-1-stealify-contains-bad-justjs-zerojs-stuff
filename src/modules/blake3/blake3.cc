...
void stealify::blake3::HashInit(const FunctionCallbackInfo<Value> &args) {
...
void stealify::blake3::HashInitKeyed(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<ArrayBuffer> ab = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> buf = ab->GetBackingStore();
...
void stealify::blake3::HashUpdate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::blake3::HashFinalize(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<ArrayBuffer> outab = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> out = outab->GetBackingStore();
...
void stealify::blake3::Init(Isolate* isolate, Local<ObjectTemplate> target) {
