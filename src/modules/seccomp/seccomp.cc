...
void stealify::seccomp::Create(const FunctionCallbackInfo<Value> &args) {
...
void stealify::seccomp::AddRule(const FunctionCallbackInfo<Value> &args) {
...
void stealify::seccomp::Load(const FunctionCallbackInfo<Value> &args) {
...
void stealify::seccomp::Release(const FunctionCallbackInfo<Value> &args) {
	seccomp_release((scmp_filter_ctx)args[0].As<Object>()->GetAlignedPointerFromInternalField(0));
}

void stealify::seccomp::GetName(const FunctionCallbackInfo<Value> &args) {
...
void stealify::seccomp::GetNumber(const FunctionCallbackInfo<Value> &args) {
  Isolate* isolate = args.GetIsolate();
...
void stealify::seccomp::Init(Isolate* isolate, Local<ObjectTemplate> target) {
