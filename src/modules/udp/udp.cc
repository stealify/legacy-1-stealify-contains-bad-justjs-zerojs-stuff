...
void stealify::udp::RecvMsg(const FunctionCallbackInfo<Value> &args) {
...
void stealify::udp::SendMsg(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::udp::Init(Isolate* isolate, Local<ObjectTemplate> target) {
