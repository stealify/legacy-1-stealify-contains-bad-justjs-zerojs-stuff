...
void stealify::epoll::EpollCtl(const FunctionCallbackInfo<Value> &args) {
...
void stealify::epoll::EpollCreate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::epoll::EpollWait(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  int loopfd = args[0]->Int32Value(context).ToChecked();
...
void stealify::epoll::Init(Isolate* isolate, Local<ObjectTemplate> target) {
