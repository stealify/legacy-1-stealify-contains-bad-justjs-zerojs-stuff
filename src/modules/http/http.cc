...
void stealify::http::GetUrl(const FunctionCallbackInfo<Value> &args) {
...
void stealify::http::GetMethod(const FunctionCallbackInfo<Value> &args) {
  int index = Local<Integer>::Cast(args[0])->Value();
...
void stealify::http::GetMethodAndUrl(const FunctionCallbackInfo<Value> &args) {
...
void stealify::http::GetStatusCode(const FunctionCallbackInfo<Value> &args) {
  int index = Local<Integer>::Cast(args[0])->Value();
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), state[index].status));
}

void stealify::http::GetVersion(const FunctionCallbackInfo<Value> &args) {
  int index = Local<Integer>::Cast(args[0])->Value();
  args.GetReturnValue().Set(Integer::New(args.GetIsolate(), state[index].minor_version));
}

void stealify::http::GetStatusMessage(const FunctionCallbackInfo<Value> &args) {
  int index = Local<Integer>::Cast(args[0])->Value();
...
void stealify::http::GetHeaders(const FunctionCallbackInfo<Value> &args) {
  Isolate* isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  int index = Local<Integer>::Cast(args[0])->Value();
...
void stealify::http::GetRequests(const FunctionCallbackInfo<Value> &args) {
  Isolate* isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::http::GetResponses(const FunctionCallbackInfo<Value> &args) {
  Isolate* isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  int count = Local<Integer>::Cast(args[0])->Value();
...
void stealify::http::ParseRequests(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  ...
  int count = 0;
  state[count].num_headers = STEALIFY_MAX_HEADERS;
  //TODO: buffer overrun
...
void stealify::http::ParseResponses(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
  size_t bytes = Local<Integer>::Cast(args[1])->Value();
  size_t off = Local<Integer>::Cast(args[2])->Value();
...
void stealify::http::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  ...
  SET_VALUE(isolate, http, "MAX_HEADERS", Integer::New(isolate, STEALIFY_MAX_HEADERS));
