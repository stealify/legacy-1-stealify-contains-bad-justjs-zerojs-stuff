...
static void ResetHandleAndSetFlag(const v8::WeakCallbackInfo<stealify::memory::rawBuffer>& data) {
  data.GetParameter()->buffer.Reset();
  stealify::memory::buffers.erase(data.GetParameter()->handle);
  fprintf(stderr, "free %i\n", data.GetParameter()->handle);
  for ( const auto &p : stealify::memory::buffers ) {
    fprintf(stderr, "%i\n", p.first);
  } 
  delete data.GetParameter();
}

void stealify::memory::ReadMemory(const FunctionCallbackInfo<Value> &args) {
...
void stealify::memory::GetMeta(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
...
void stealify::memory::RawBuffer(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  Local<ArrayBuffer> ab = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> backing = ab->GetBackingStore();
  stealify::memory::rawBuffer* buf = new stealify::memory::rawBuffer();
  buf->data = backing->Data();
  buf->len = backing->ByteLength();
  buf->buffer.Reset(isolate, ab);
  buf->handle = stealify::memory::bcount;
  buf->buffer.SetWeak(buf, &ResetHandleAndSetFlag, v8::WeakCallbackType::kFinalizer);
  stealify::memory::buffers[stealify::memory::bcount] = buf;
  args.GetReturnValue().Set(Integer::New(isolate, stealify::memory::bcount++));
}

void stealify::memory::Alloc(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  ...
  buf->buffer.SetWeak(buf, &ResetHandleAndSetFlag, v8::WeakCallbackType::kFinalizer);
  stealify::memory::buffers[stealify::memory::bcount] = buf;
  ab->Set(context, String::NewFromUtf8Literal(isolate, "raw", NewStringType::kNormal), Integer::New(isolate, stealify::memory::bcount++)).Check();
  args.GetReturnValue().Set(ab);
}

void stealify::memory::WriteString(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  stealify::memory::rawBuffer* b = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
...
void stealify::memory::WriteCString(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  stealify::memory::rawBuffer* b = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
  Local<String> str = args[1].As<String>();
  int len = str->Length();
...
void stealify::memory::WritePointer(const FunctionCallbackInfo<Value> &args) {
  stealify::memory::rawBuffer* dest = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
  int off = Local<Integer>::Cast(args[1])->Value();
  stealify::memory::rawBuffer* src = stealify::memory::buffers[Local<Integer>::Cast(args[2])->Value()];
  char* ptr = (char*)dest->data + off;
  *reinterpret_cast<void **>(ptr) = src->data;
}

void stealify::memory::ReadString(const FunctionCallbackInfo<Value> &args) {
  stealify::memory::rawBuffer* b = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
  int len = b->len;
...
void stealify::memory::Copy(const FunctionCallbackInfo<Value> &args) {
  stealify::memory::rawBuffer* dest = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
  stealify::memory::rawBuffer* src = stealify::memory::buffers[Local<Integer>::Cast(args[1])->Value()];
  int argc = args.Length();
  int doff = 0;
  if (argc > 2) {
...
void stealify::memory::GetAddress(const FunctionCallbackInfo<Value> &args) {
  stealify::memory::rawBuffer* b = stealify::memory::buffers[Local<Integer>::Cast(args[0])->Value()];
  args.GetReturnValue().Set(BigInt::New(args.GetIsolate(), (uint64_t)b->data));
}

void stealify::memory::MemFdCreate(const FunctionCallbackInfo<Value> &args) {
  Isolate* isolate = args.GetIsolate();
...
void stealify::memory::Init(Isolate* isolate, Local<ObjectTemplate> target) {
