...
void stealify::signal::SignalFD(const FunctionCallbackInfo<Value> &args) {
...
void stealify::signal::SigEmptySet(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> backing = buf->GetBackingStore();
...
void stealify::signal::SignalHandler(int signal, siginfo_t* info, void* void_context) {
...
void stealify::signal::SigAction(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  ...
  if (args.Length() > 1) {
    ...
    stealify::signal::signalHandler* handler = new stealify::signal::signalHandler();
    stealify::signal::handlers[sig] = handler;
    handler->signum = sig;
...
void stealify::signal::Reset(const FunctionCallbackInfo<Value> &args) {
  struct sigaction act;
  memset(&act, 0, sizeof(act));
  int r = 0;
...
void stealify::signal::SigFillSet(const FunctionCallbackInfo<Value> &args) {
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> backing = buf->GetBackingStore();
  sigset_t* set = static_cast<sigset_t*>(backing->Data());
...
void stealify::signal::SigWait(const FunctionCallbackInfo<Value> &args) {
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
  std::shared_ptr<BackingStore> backing = buf->GetBackingStore();
  sigset_t* set = static_cast<sigset_t*>(backing->Data());
...
void stealify::signal::SigProcMask(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
...
void stealify::signal::SigAddSet(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<ArrayBuffer> buf = args[0].As<ArrayBuffer>();
...
void stealify::signal::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  ...
  SET_VALUE(isolate, module, "STEALIFY_SIGSAVE", Integer::New(isolate, 1));
  SET_VALUE(isolate, module, "STEALIFY_SIGLOAD", Integer::New(isolate, 0));
