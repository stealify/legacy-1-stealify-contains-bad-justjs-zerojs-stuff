...
void stealify::zlib::FreeMemory(void* buf, size_t length, void* data) {
  //fprintf(stderr, "free: %lu\n", length);
}

void stealify::zlib::Crc32(const FunctionCallbackInfo<Value> &args) {
...
void stealify::zlib::WriteDeflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<ArrayBuffer> ab = args[0].As<ArrayBuffer>();
...
void stealify::zlib::WriteInflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::zlib::EndDeflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<ArrayBuffer> ab = args[0].As<ArrayBuffer>();
  z_stream* stream = (z_stream*)ab->GetAlignedPointerFromInternalField(1);
...
void stealify::zlib::EndInflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<ArrayBuffer> ab = args[0].As<ArrayBuffer>();
  z_stream* stream = (z_stream*)ab->GetAlignedPointerFromInternalField(1);
...
void stealify::zlib::CreateDeflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::zlib::CreateInflate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  z_stream* stream = (z_stream*)calloc(1, sizeof(z_stream));
...
void stealify::zlib::Init(Isolate* isolate, Local<ObjectTemplate> target) {
