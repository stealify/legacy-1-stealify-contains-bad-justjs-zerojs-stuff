...
void stealify::tcc::Compile(const FunctionCallbackInfo<Value> &args) {
...
void stealify::tcc::GetSymbol(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::tcc::AddSymbol(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<BigInt> address64 = Local<BigInt>::Cast(args[0]);
  TCCState* s = reinterpret_cast<TCCState*>(address64->Uint64Value());
...
void stealify::tcc::Destroy(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<BigInt> address64 = Local<BigInt>::Cast(args[0]);
  TCCState* s = reinterpret_cast<TCCState*>(address64->Uint64Value());
...
void stealify::tcc::Relocate(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<BigInt> address64 = Local<BigInt>::Cast(args[0]);
  TCCState* s = reinterpret_cast<TCCState*>(address64->Uint64Value());
...
void stealify::tcc::Init(Isolate* isolate, Local<ObjectTemplate> target) {
