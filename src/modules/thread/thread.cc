...
void* stealify::thread::startThread(void *data) {
  threadContext* ctx = (threadContext*)data;
  stealify::CreateIsolate(ctx->argc, ctx->argv, ctx->main, 
...
void stealify::thread::Spawn(const FunctionCallbackInfo<Value> &args) {
  ...
  ctx->buf.iov_len = 0;
  ctx->fd = 0;
  // we can pass in a set of args for stealify.args
  if (argc > 2) {
  ...
  // shared array buffer. will be in stealify.buffer variable inside thread
  if (argc > 3) {
...
void stealify::thread::Join(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::thread::TryJoin(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<BigInt> bi = args[0]->ToBigInt(context).ToLocalChecked();
...
void stealify::thread::Self(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
...
void stealify::thread::SetAffinity(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<BigInt> bi = args[0]->ToBigInt(context).ToLocalChecked();
...
void stealify::thread::SetName(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<BigInt> bi = args[0]->ToBigInt(context).ToLocalChecked();
...
void stealify::thread::GetAffinity(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
  Local<BigInt> bi = args[0]->ToBigInt(context).ToLocalChecked();
...
void stealify::thread::Init(Isolate* isolate, Local<ObjectTemplate> target) {
