  #include "ffi_stealify.h"

static ffi_type* stealify_ffi_types[16];

void stealify::ffi::FfiPrepCif(const FunctionCallbackInfo<Value> &args) {
  ...
  for (unsigned int i = 0; i < nargs; i++) {
    ...
    argtypes[i] = stealify_ffi_types[Local<Integer>::Cast(p)->Value()];
  }
  status = ffi_prep_cif(cif, abi, nargs, stealify_ffi_types[rtype], argtypes);
  if (status != FFI_OK) {
...
void stealify::ffi::FfiCall(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  ...
  if (cif->rtype == stealify_ffi_types[FFI_TYPE_UINT32]) {
    args.GetReturnValue().Set(Integer::New(isolate, (uint32_t)result));
    return;
  }
  if (cif->rtype == stealify_ffi_types[FFI_TYPE_DOUBLE]) {
    args.GetReturnValue().Set(v8::Number::New(isolate, (double)result));
    return;
  }
  if (cif->rtype == stealify_ffi_types[FFI_TYPE_POINTER]) {
    args.GetReturnValue().Set(BigInt::New(isolate, (uint64_t)result));
    return;
  }
}

void stealify::ffi::Init(Isolate* isolate, Local<ObjectTemplate> target) {
  Local<ObjectTemplate> module = ObjectTemplate::New(isolate);
  SET_METHOD(isolate, module, "ffiPrepCif", FfiPrepCif);
  SET_METHOD(isolate, module, "ffiCall", FfiCall);

  stealify_ffi_types[FFI_TYPE_UINT32] = &ffi_type_uint32;
  stealify_ffi_types[FFI_TYPE_DOUBLE] = &ffi_type_double;
  stealify_ffi_types[FFI_TYPE_POINTER] = &ffi_type_pointer;
  stealify_ffi_types[FFI_TYPE_VOID] = &ffi_type_void;
  stealify_ffi_types[FFI_TYPE_UINT64] = &ffi_type_uint64;
  stealify_ffi_types[FFI_TYPE_SINT32] = &ffi_type_sint32;
  stealify_ffi_types[FFI_TYPE_COMPLEX] = &ffi_type_complex_double;
