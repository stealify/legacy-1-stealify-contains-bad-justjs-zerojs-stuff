...
/*
void stealify::sys::FindFast(const FunctionCallbackInfo<Value> &args) {
...
void stealify::html::Escape(const FunctionCallbackInfo<Value> &args) {
  Isolate *isolate = args.GetIsolate();
  HandleScope handleScope(isolate);
  Local<Context> context = isolate->GetCurrentContext();
...
void stealify::html::Init(Isolate* isolate, Local<ObjectTemplate> target) {
