  ...
  function onConnect (fd, event) {
    ...
    loop.add(clientfd, (fd, event) => {
      ...
      if (bytes < 0) {
        ...
        stealify.error(`recv error: ${sys.strerror(errno)} (${errno})`)
      }
      closeSocket(socket)
    })
    ...
    socket.write = (buf, len = byteLength, off = 0) => {
      ...
      if (written < 0) {
        ...
        stealify.error(`write error (${clientfd}): ${sys.strerror(errno)} (${errno})`)
      }
      if (written === 0) {
        stealify.error(`zero write ${clientfd}`)
      }
      return written
    }
...
function createClient (path) {
  ...
  function handleError (fd, event) {
    ...
    if (!sock.connected) {
      sock.onConnect(new Error(`${errno} : ${stealify.sys.strerror(errno)}`))
    }
  }
  ...
  sock.write = (buf, len = buf.byteLength, off = 0) => {
    ...
    if (written < 0) {
      const errno = sys.errno()
      if (errno === net.EAGAIN) return written
      stealify.error(`write error (${fd}): ${sys.strerror(errno)} (${errno})`)
    }
    if (written === 0) {
      stealify.error(`zero write ${fd}`)
    }
    return written
  }
