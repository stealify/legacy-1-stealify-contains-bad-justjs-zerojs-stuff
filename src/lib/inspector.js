...
const defaultOpts = {
  host: '127.0.0.1',
  port: 9222,
  favicon: 'https://stealify.billywhizz.io/favicon.ico',
  onReady: () => {}
}
...
function createInspector (opts = defaultOpts) {
  ...
  function sha1 (str) {
    ...
    return source.readString(encode.base64Encode(dest, source, stealify.sha1.hash(source, dest)))
  }
  ...
  function startWebSocket (request) {
    ...
    parser.onMessage = header => {
      ...
      if (!str) return
      //stealify.error(`${AG}global.send${AD}\n${str}`)
      global.send(str)
      chunks.length = 0
      try {
        ...
        //stealify.error(`${AG}inspector.parse.error${AD}\n${str}`)
        stealify.error(err.stack)
      }
    }
  ...
  function getJSON () {
    ...
    if (stealify.args.length > 1) {
      fileName = stealify.args[1].split('/').slice(-1)[0]
    }
    res.push('HTTP/1.1 200 OK')
    res.push('Content-Type: application/json; charset=UTF-8')
    res.push('Cache-Control: no-cache')
    const payload = JSON.stringify([{
      ...
      title: fileName,
      type: 'node',
      url: `file://${stealify.sys.cwd()}/${stealify.args[1]}`,
  ...
  function onSocketEvent (fd, event) {
    ...
    if (bytes > 0) {
      ...
      if (err < 0) stealify.print(`error: ${err}`)
      return
    }
    if (bytes < 0) {
      const errno = sys.errno()
      if (errno !== net.EAGAIN) {
        stealify.print(`recv error: ${sys.strerror(errno)} (${errno})`)
        net.close(fd)
        delete clients[fd]
      }
      return
  ...
  global.receive = message => {
    //stealify.error(`${AM}global.receive${AD}\n${message}`)
  ...
  const { encode, sys, net } = stealify
  const clients = {}
  const { EPOLLIN, EPOLLERR, EPOLLHUP } = stealify.loop
  const {
    SOMAXCONN,
    O_NONBLOCK,
    SOCK_STREAM,
