const { net, sys, vm } = stealify
const { EPOLLERR, EPOLLHUP } = stealify.loop
const { errno } = stealify.sys
...
function repl (loop = stealify.factory.loop, buf = new ArrayBuffer(4096), stdin = sys.STDIN_FILENO, stdout = sys.STDOUT_FILENO) {
  const { EPOLLIN } = stealify.loop
  const { O_NONBLOCK, EAGAIN } = stealify.net
  ...
  let r = loop.add(stdin, (fd, event) => {
    ...
    if (event & EPOLLIN) {
      ...
      if (bytes < 0) {
        const err = sys.errno()
        if (err !== EAGAIN) {
          stealify.print(`read error: ${sys.strerror(err)} (${err})`)
          net.close(fd)
        }
        return
      }
  ...
  if (r !== 0) throw new stealify.SystemError('stdin.add')
  r = loop.add(stdout, (fd, event) => {
    // todo: pause/resume on writable
  })
  if (r !== 0) throw new stealify.SystemError('stderr.add')
  writeString(stdout, `${ANSI_GREEN}>${ANSI_DEFAULT} `)
  return r
}
