const { print, error, builtin } = stealify
const { launch, watch } = stealify.process
const { cwd } = stealify.sys
const { compile } = stealify.vm
const { unlink, isDir, isFile, writeFile, mkdir, rmdir, readDir, chdir, rename } = stealify.fs
...
function generateBuiltins (main, index, libs, embeds, modules, v8flags, v8flagsFromCommandLine = false, stealifyDir = '') {
  const lines = []
  const files = libs.concat(embeds)
  // todo - make this a string literal/template
  for (let file of files) {
    if (file[0] === '/') {
      file = file.replace(`${stealifyDir}/`, '')
    }
  ...
  for (let file of files) {
    ...
    const path = file.replace(/[./]/g, '_')
    lines.push(`  stealify::builtins_add("${file}", _binary_${path}_start, _binary_${path}_end - _binary_${path}_start);`)
  }
  for (const module of modules) {
    if (module.exports) {
      for (const e of module.exports) {
        lines.push(`  stealify::modules["${e.name}"] = &_register_${e.name};`)
      }
    } else {
      lines.push(`  stealify::modules["${module.name}"] = &_register_${module.name};`)
    }
  }
  lines.push('}')
  let path = main.replace(/[./]/g, '_')
  lines.push(`static unsigned int stealify_js_len = _binary_${path}_end - _binary_${path}_start;`)
  lines.push(`static const char* stealify_js = _binary_${path}_start;`)
  if (index) {
...
async function run (config = {}, { cleanall = false, clean = false, dump = false, silent = false }) {
  ...
  if (config.target === 'stealify') {
    ...
    for (const fileName of config.libs) {
      if (fileName[0] === '/') {
        links[fileName] = linkFile(fileName, fileName.replace(`${stealifyDir}/`, ''))
      } else {
        if (isFile(`${appDir}/${fileName}`)) {
  ...
  config.stealifyDir = stealifyDir
  if (dump) {
    config.build = build
  ...
  if (config.target !== 'stealify') {
    if (!isDir(stealifyDir)) mkdir(stealifyDir)
    chdir(stealifyDir)
  }
  if (config.main === 'stealify.js') {
    if (!isFile('stealify.js')) writeFile('stealify.js', ArrayBuffer.fromString(builtin('stealify.js')))
  ...
  const src = generateBuiltins(main, index, config.libs, config.embeds, modules, v8flags, v8flagsFromCommandLine, stealifyDir)
  ...
  if (config.target !== 'stealify') {
    chdir(appDir)
    rename(`${stealifyDir}/${target}`, `${appDir}/${target}`)
  }
}
...
function init (name) {
  ...
  safeWrite(`${name}.js`, 'stealify.print(stealify.memoryUsage(new BigUint64Array(16))[0])\n')
  chdir('../')
}
