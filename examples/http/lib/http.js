const { http } = stealify.library('http', 'http.so')
const {
  parseResponses,
  getResponses,
  parseRequests,
    ...
    while (bytes) {
      if (inHeader) {
        ...
        if (c === 13) {
          ...
          stealify.print('BAD_CHAR')
        }
        stealify.print('OOB:')
        stealify.print(`c ${c}`)
        stealify.print(`bytes ${bytes}`)
        stealify.print(`offset ${offset}`)
        stealify.print(`chunkLen ${chunkLen}`)
        throw new Error('OOB')
      } else {
        if (bytes >= chunkLen) {
          buf.offset = offset
...
function createHTTPServer (server, name = 'j') {
  if (server.timer) stealify.clearInterval(server.timer)
  ...
  server.timer = stealify.setInterval(() => {
