...
function main () {
  ...
  server.onConnect = sock => {
    ...
    // abstraction. this is stealify to show various bits of lower level plumbing
    const END = '\r\n\r\n'
    ...
    sock.onClose = () => {
      if (sock.timer) stealify.clearTimeout(sock.timer)
      parser.free()
    }
  ...
  const onRequest = (req, sock) => {
    if (req.method === 'GET') {
      if (req.url === '/json') {
        if (sock.json(JSON.stringify(req)) < 0) stealify.error((new SystemError('sock.json')).stack)
        return
      }
      if (req.url === '/async') {
        ...
        sock.timer = stealify.setTimeout(() => {
          if (sock.json(JSON.stringify(req)) < 0) stealify.error((new SystemError('sock.json')).stack)
        }, 1000)
        return
      }
      if (req.url === '/favicon.ico') {
        if (sock.favicon(favicon) < 0) stealify.error((new SystemError('sock.favicon')).stack)
        return
      }
    }
    if (sock.writeString(server.r404) < 0) stealify.error((new SystemError('sock.404')).stack)
  }
  let err = server.bind()
