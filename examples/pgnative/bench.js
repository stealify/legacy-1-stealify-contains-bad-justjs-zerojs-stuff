...
stealify.setInterval(() => {
  ...
  last.user = user
  last.system = system
  stealify.print(`qps ${qps} clients ${clients} mem ${rss} cpu (${upc.toFixed(2)}/${spc.toFixed(2)}) ${(upc + spc).toFixed(2)} qps/core ${(qps / (upc + spc)).toFixed(2)}`)
  qps = 0
}, 1000)
...
let todo = parseInt(stealify.args[2] || '1', 10)
