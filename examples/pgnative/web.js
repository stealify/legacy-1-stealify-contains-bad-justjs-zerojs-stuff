...
stealify.setInterval(() => {
  ...
  if (u32) {
    ...
    last.user = user
    last.system = system
    stealify.print(`qps ${qps} rps ${rps} clients ${clients.length} mem ${rss} cpu (${upc.toFixed(2)}/${spc.toFixed(2)}) ${(upc + spc).toFixed(2)} qps/core ${(qps / (upc + spc)).toFixed(2)}`)
  }
  qps = 0
  rps = 0
}, 1000)
...
const connections = parseInt(stealify.args[2] || '1', 10)
let i = connections
...
function startup () {
  stealify.print(`startup ${clients.length}`)
