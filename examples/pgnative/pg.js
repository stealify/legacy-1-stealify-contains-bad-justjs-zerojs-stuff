const { crypto } = stealify.library('openssl.so', 'crypto')
const { net, sys, encode } = stealify
const { AF_INET, SOCK_STREAM, SOCK_NONBLOCK, SOL_SOCKET, IPPROTO_TCP, TCP_NODELAY, SO_KEEPALIVE, SO_ERROR } = net
const { EPOLLERR, EPOLLHUP, EPOLLIN, EPOLLOUT, EPOLLET } = stealify.loop
...
function readCString (buf, u8, off) {
  const start = off
  while (u8[off]) off++
  return stealify.sys.readString(buf, off - start, start)
}
...
function onSocketEvent (fd, event) {
  ...
  if (event & EPOLLERR || event & EPOLLHUP) {
    ...
    delete clients[fd]
    if (!client.connected) {
      client.onConnect(new Error(`${errno} : ${stealify.sys.strerror(errno)}`))
    }
    return
  }
  if (event & EPOLLIN) {  
