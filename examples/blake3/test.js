const { blake3 } = stealify.library('blake3.so', 'blake3')
const { create, update, finish, BLAKE3_OUT_LEN } = blake3
const { sys, net, encode } = stealify
...
while (1) {
  ...
  if (bytes < 0) {
    const err = errno()
    stealify.print(`read error: ${strerror(err)} (${err})`)
    close(stdin)
    break
  }
  if (bytes === 0) {
    ...
    stealify.print(`bytes ${total} blake3 ${str} ${toGib(total / seconds)} Gbit/sec rss ${stealify.memoryUsage().rss}`)
    close(stdin)
    break
  }
  total += bytes
