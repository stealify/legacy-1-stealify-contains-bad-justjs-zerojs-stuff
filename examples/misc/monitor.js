...
stealify.setInterval(() => {
  ...
  const rss = Math.floor((rssPages * stealify.sys.pageSize) / (1024 * 1024))
  ...
  stealify.print(`${ANSI_GREEN}rss${ANSI_DEFAULT} ${format(rss)} ${ANSI_GREEN}usr${ANSI_DEFAULT} ${format(user)} ${ANSI_GREEN}sys${ANSI_DEFAULT} ${format(system)} ${ANSI_GREEN} tot${ANSI_DEFAULT} ${format(system + user)} ${ANSI_GREEN}cusr${ANSI_DEFAULT} ${format(cuser)} ${ANSI_GREEN}csys${ANSI_DEFAULT} ${format(csystem)} ${ANSI_GREEN} ctot${ANSI_DEFAULT} ${format(csystem + cuser)}`)
}, 1000)

const pid = parseInt(stealify.args[2] || stealify.sys.pid())
