...
function runSmall (runs, fun) {
  ...
  stealify.print(`small.${fun.name}  ${rate}`)
}
...
function runLarge (runs, fun) {
  ...
  const elapsed = Date.now() - then
  const rate = Math.ceil((runs / (elapsed / 1000)))
  stealify.print(`large.${fun.name}  ${rate}`)
}

function run () {
  runLarge(parseInt(stealify.args[2] || '1000000', 10), String.byteLength)
  runSmall(parseInt(stealify.args[3] || '100000000', 10), String.byteLength)
  stealify.setTimeout(run, 100)
}
