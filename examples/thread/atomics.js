stealify.thread = stealify.library('thread').thread
const { net, sys, loop } = stealify
function threadMain () {
  const shared = stealify.buffer
  const u32 = new Uint32Array(shared)
  Atomics.add(u32, 0, 1)
  stealify.setInterval(() => {}, 1000)
}
...
const u32 = new Uint32Array(shared)
const main = stealify.builtin('stealify.js')
...
while (Atomics.load(u32, 0) < 10000) {
  ...
  for (let j = 0; j < parallel; j++) {
    tids.push(stealify.thread.spawn(source, main, [], shared))
  }
  ...
  for (let i = 0; i < r; i++) {
    ...
    if (event & loop.EPOLLIN) {
      ...
      stealify.print(`mem ${rss} diff ${diff} rate ${(rate / elapsed).toFixed(2)} counter ${Atomics.load(u32, 0)}`)
      net.read(fd, tbuf)
      rate = 0
      then = now
    }
