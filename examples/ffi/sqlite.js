...
const handle = stealify.sys.dlopen('libsqlite3.so')

if (!handle) throw new Error('Clould not create handle')

function strlen (ptr) {
  const fn = stealify.sys.dlsym(handle, 'strlen')
...
function version () {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_libversion')
  if (!fn) throw new Error('Could not find symbol')
  const cif = new ArrayBuffer(8)
  ...
  return stealify.sys.readMemory(ptr, ptr + BigInt(len)).readString(len)
}

function open (fileName) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_open')
  if (!fn) throw new Error('Could not find symbol')
...
function close (address) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_close')
  if (!fn) throw new Error('Could not find symbol')
  const cif = new ArrayBuffer(16)
  const dv = new DataView(cif)
  const status = ffi.ffiPrepCif(cif, ffi.FFI_TYPE_UINT32, [ffi.FFI_TYPE_POINTER])
...
function prepare (db, sql) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_prepare_v3')
  if (!fn) throw new Error('Could not find symbol')
...
function step (address) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_step')
  if (!fn) throw new Error('Could not find symbol')
  const cif = new ArrayBuffer(16)
  const dv = new DataView(cif)
  const status = ffi.ffiPrepCif(cif, ffi.FFI_TYPE_UINT32, [ffi.FFI_TYPE_POINTER])
...
function finalize (address) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_finalize')
  if (!fn) throw new Error('Could not find symbol')
  const cif = new ArrayBuffer(16)
  const dv = new DataView(cif)
  const status = ffi.ffiPrepCif(cif, ffi.FFI_TYPE_UINT32, [ffi.FFI_TYPE_POINTER])
...
function exec (address, sql) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_exec')
  if (!fn) throw new Error('Could not find symbol')
...
function column_text (address, index) {
  const fn = stealify.sys.dlsym(handle, 'sqlite3_column_text')
  if (!fn) throw new Error('Could not find symbol')
  const cif = new ArrayBuffer(16)
  const dv = new DataView(cif)
  ...
  const ptr = ffi.ffiCall(cif, fn)
  const len = strlen(ptr)
  return stealify.sys.readMemory(ptr, ptr + BigInt(len))
}

let r = 0
stealify.print(`version ${version()}`)
...
try {
  ...
  stealify.print(`prepare ${res.status}`)
  stealify.print(res.stmt)
  stealify.print(res.ztail)
  r = step(res.stmt)
  stealify.print(`step ${r}`)
  r = finalize(res.stmt)
  stealify.print(`finalize ${r}`)
  stealify.print('table created')
} catch (err) {
  stealify.print('creating table failed')
}
...
try {
  ...
  stealify.print('inserting record failed')
}
...
try {
  ...
  while (r === constants.SQLITE_ROW) {
    ...
    stealify.print(`column_text ${str}`)
  ...
  r = finalize(res.stmt)
  stealify.print(`finalize ${r}`)
  stealify.print(`total rows ${rows}`)
} catch (err) {
  stealify.print('inserting record failed')
}

r = close(db)
stealify.print(`close ${r}`)
