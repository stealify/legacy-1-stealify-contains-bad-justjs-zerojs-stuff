...
const { udp } = stealify.library('udp')
      ...
      socket.parser.onResponses = count => {
        for (const res of socket.parser.get(count)) {
          ...
          if (res.statusCode === 200) {
            ...
            stealify.print(`redirect ${location}`)
          }
        }
      }
      socket.onSecure()
  ...
  if (event & EPOLLIN) {
    ...
    if (bytes < 0) {
      ...
      if (err === tls.SSL_ERROR_WANT_READ) {
        ...
        stealify.print(`tls error ${err}: ${sys.errno()}: ${sys.strerror(sys.errno())}`)
      }
      return
    }
    const err = tls.error(buf, bytes)
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.onResponse = res => {
      if (res.statusCode === 200) {
        ...
        res.file = file
        file.fd = stealify.fs.open(fileName, stealify.fs.O_WRONLY | stealify.fs.O_CREAT | stealify.fs.O_TRUNC)
        if (file.fd < 3) return socket.close(new Error(`failed to open output file ${fileName}`))
        socket.onBody = bytes => {
          stealify.net.write(file.fd, buf, bytes, buf.offset)
          res.file.size += bytes
        }
