const { rocksdb } = stealify.library('../../modules/rocksdb/rocksdb.so', 'rocksdb')
const {
  open,
  get,
  put,
...
function benchPut (count = 100000) {
  ...
  stealify.print(`put rate ${rate} rss ${stealify.memoryUsage().rss}`)
  stealify.setTimeout(benchPut, 10)
}
...
function benchGet (count = 100000) {
  ...
  const elapsed = (Date.now() - start) / 1000
  const rate = count / elapsed
  stealify.print(`get rate ${rate} rss ${stealify.memoryUsage().rss}`)
  stealify.setTimeout(benchGet, 10)
}
