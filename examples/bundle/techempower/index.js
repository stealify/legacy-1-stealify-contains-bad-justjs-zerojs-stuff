...
function onPGConnect (err, sock) {
  if (err) {
    ...
    stealify.setTimeout(() => connect(tfb, onPGConnect), 1000)
    return
  }
  sock.onClose = () => {
    // todo: remove from pool and reconnect?
    stealify.error('pg.close')
  }
  sock.start(err => {
    if (err) return stealify.error(err.stack)
    sock.authenticate(err => {
      if (err) return stealify.error(err.stack)
      onPGAuth(sock).catch(err => stealify.error(err.stack))
    })
  })
}
...
function onPGReady () {
  microtasks = false
  stealify.print(`listen: ${server.listen()}`)
}

const { utf8Length } = stealify.sys
const poolSize = parseInt(stealify.env().PGPOOL || stealify.sys.cpus, 10)
...
stealify.print(`version ${require('config.js').version}`)
while (1) {
  if (loop.poll(0) === 0) loop.poll(-1)
  if (microtasks) stealify.sys.runMicroTasks()
}
