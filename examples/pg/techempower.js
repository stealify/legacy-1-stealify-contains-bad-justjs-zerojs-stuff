const { createServer } = stealify.require('./lib/transport.js')
const { createParser } = stealify.require('./lib/protocol.js')
...
function connect () {
  ...
  if (!handle) {
    stealify.error('could not connect')
    stealify.setTimeout(connect, 1000)
    return
  }
  server.listen()
}
...
function onStats () {
  const { user, system } = stealify.cpuUsage()
  const { rss } = stealify.memoryUsage()
  const upc = ((user - last.user) / 1000000)
  const spc = ((system - last.system) / 1000000)
  last.user = user
  last.system = system
  stealify.print(`qps ${qps} mem ${rss} cpu (${upc.toFixed(2)}/${spc.toFixed(2)}) ${(upc + spc).toFixed(2)} qps/core ${(qps / (upc + spc)).toFixed(2)}`)
  r404 = `HTTP/1.1 404 Not Found\r\nServer: stealify\r\nDate: ${time}\r\nContent-Type: text/plain\r\nContent-Length: 0\r\n\r\n`
  qps = 0
}
...
connect()
if (stealify.buffer) {
  u32 = new Uint32Array(stealify.buffer)
  stealify.setInterval(onClock, 100)
} else {
  stealify.setInterval(onStats, 1000)
}
