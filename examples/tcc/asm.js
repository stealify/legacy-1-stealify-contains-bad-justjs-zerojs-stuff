const { tcc } = stealify.library('tcc')
...
ssize_t stealify_write(int fd, const void *buf, size_t size)
{
    ssize_t ret;
    asm volatile
    (
...
function wrapWrite (buf, fd) {
  const fn = tcc.get(code, 'stealify_write')
  ...
  function write (fd, str, len = stealify.sys.utf8Length(str)) {
    fdv.setUint32(0, fd, true)
...
function ffiWriteString () {
  ...
  let i = 0
  const str = 'hello\n'
  const len = stealify.sys.utf8Length(str)
  const fd = stealify.sys.STDERR_FILENO
  const start = Date.now()
  while (write(fd, str, len) === 6) {
    if (i++ === 1000000) break
  }
  stealify.print(Date.now() - start)
}

function nativeWriteString () {
  const write = stealify.net.writeString
  let i = 0
  const str = 'hello\n'
  const fd = stealify.sys.STDERR_FILENO
  const start = Date.now()
...
function ffiWriteBuffer () {
  ...
  const { write2 } = wrapWrite(buf, stealify.sys.STDERR_FILENO)
  let i = 0
  const start = Date.now()
  while (write2() === 6) {
    if (i++ === 1000000) break
...
function nativeWriteBuffer () {
  const write = () => stealify.net.write(fd, buf)
  let i = 0
  const str = 'hello\n'
...
stealify.net.dup(stealify.fs.open('/dev/null', stealify.fs.O_RDWR), stealify.sys.STDERR_FILENO)
const runs = parseInt(stealify.args[2] || '10')
