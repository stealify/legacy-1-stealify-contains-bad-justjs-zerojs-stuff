...
function onPacket (packet, u8) {
  ...
  if (frame && frame.protocol === 'IPv4' && header && header.protocol === protocols.TCP) {
    // tcp frames
    stealify.print(tcpDump(packet))
    if (bytes > offset) stealify.print(dump(u8.slice(offset, bytes)), false)
    stealify.print('')
  } else if (frame && frame.protocol === 'IPv4' && header && header.protocol === protocols.UDP) {
    // handle a udp message
    stealify.print(udpDump(packet))
    if (bytes > offset) stealify.print(dump(u8.slice(offset, bytes)), false)
    stealify.print('')
  }
}

const { net, SystemError } = stealify
...
function main (args) {
  ...
  if (iff) {
    ...
    stealify.print(`bound to interface ${iff} (${toMAC(new Uint8Array(mac))})`)
  }
  while (1) {
