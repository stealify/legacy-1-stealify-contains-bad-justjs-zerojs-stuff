const { net } = stealify
const { EPOLLIN, EPOLLOUT, EPOLLERR, EPOLLHUP } = stealify.loop
    ...
    loop.add(backend, (fd, event) => {
        ...
        loop.add(client, (fd, event) => {
          if (event & EPOLLIN) {
            ...
            if (bytes === 0) {
              ...
              stealify.print(`${ANSI_YELLOW}client -> (${bytes})${ANSI_DEFAULT}`)
              parse(buf, bytes, 'F')
      ...
      if (event & EPOLLIN) {
        ...
        if (bytes === 0) {
          ...
          stealify.print(`${ANSI_MAGENTA}client <- (${bytes})${ANSI_DEFAULT}`)
          parse(buf, bytes, 'B')
